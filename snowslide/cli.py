"""Command line interface ot the snowslide package."""

import sys
import snowslide.snowslide.snowslide as snowslide
from snowslide.snowslide.snowslide import cfg

HELP = """snowslide: command line tools.
...
def cli_program(args):
    ...
    if args[0] in ['-v', '--version']:
        print('snowslide (command line tools): ' + snowslide.__version__)
        print('License: public domain')
        print('snowslide is provided "as is", without warranty of any kind')
        return
    ...
    if args[0] in ['-ac', '--area-circle']:
        ...
        print('The area of a circle with radius {} '.format(radius) +
              'is {}'.format(snowslide.area_of_circle(radius)))
        return
