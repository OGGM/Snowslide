...
def save_plots3D(path_dem,SND_plot=None,save_path=None,
    ...
    with rasterio.open(path_dem) as ds :
        ...
        if SND_plot==None: # If the user does not enter a SND matrix it then shows only the dem
        ...
        mlab.surf(HNSO_plot, warp_scale=(1/param_camera['cellsize']),colormap='PuBu',opacity=0.9)
        ...
        distance = param_camera['distance']  # contrôle la distance à la figure, 'auto' laisse mayavi faire le choix
