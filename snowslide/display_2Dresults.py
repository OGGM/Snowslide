...
def create_giff(src_path,dst_path=None,gif_name="gif_animation",fig_names=None,n=None):
    ...
    if fig_names==None:
        ...
        images_paths = [os.path.join(src_path, file) for file in images_paths if os.path.splitext(file)[1].lower() in extensions_images and os.path.isfile(os.path.join(src_path, file))]
        images_paths = sorted(images_paths)
        
        # create gif
        n = len(images_paths)
        images = np.stack([iio.imread(images_paths[i]) for i in range(1,n+1)], axis = 0)
        if dst_path==None:
        ...
        images = np.stack([iio.imread(src_path + "/" + fig_names + f"{i}.png") for i in range(1,n+1)], axis = 0)
        iio.mimwrite(dst_path +"/" + gif_name, images)

def save_plots2D(file,save_path=None,legend=None,title=None,xlabel=None,ylabel=None,
    ...
    if save_path==None : 
        save_path = os.getcwd() # Stored in working directory if users does not indicate a saving path
