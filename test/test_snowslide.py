...
def test_mass_conservation2() :
    ...
    transform = rasterio.transform.from_origin(0, 0, 1, 1)
    crs = rasterio.crs.CRS.from_epsg(4326)
    dem_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),'data', 'linear_idealized_dem.tif')
    with rasterio.open(dem_path, "w", driver="GTiff", height=dem.shape[0], width=dem.shape[1], count=1, dtype=dem.dtype, crs=crs, transform=transform) as dst:
        dst.write(dem, 1)
...
def test_flat_holdSnow():
    ...
    crs = rasterio.crs.CRS.from_epsg(4326)
    dem_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),'data', 'idealized_linear_dem.tif')
    with rasterio.open(dem_path, "w", driver="GTiff", height=dem.shape[0], width=dem.shape[1], count=1, dtype=dem.dtype, crs=crs, transform=transform) as dst:
            dst.write(dem, 1)
...
def test_noslope_nochanges():
    ...
    transform = rasterio.transform.from_origin(0, 0, 1, 1)
    dem_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),'data', 'idealized_flat_dem.tif')
