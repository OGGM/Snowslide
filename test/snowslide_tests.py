...
def test_init_prcpt(dem,SND0) :
    """ This function verifies dem and SND0 are the same size

    Parameters
    ----------
    dem: numpy.ndarray
        Numpy matrix of altitudes 
    SND0: numpy.ndarray
    ...
    -------
    message: str
        print an assertion error message if the test is failed
    """
    
    assert np.shape(dem) == np.shape(SND0), f"The shape of the DEM is {np.shape(dem)} but you initialise the precipitation with a matrix of shape {np.shape(SND0)}. The two matrices must be the same size."

def test_DEM_isTIFF(dem_path) :
    """ This function verifies that the dem_path chosen by the user corresponds to a .tif file 

    Parameters
    ----------
    dem_path: str
        Path to the .tif dem file
